...
var x = d3.scaleBand().rangeRound([0, barContainerWidth]).padding(0.1);
barChart
  .append("g")
...
var y = d3.scaleLinear().domain([0, 12]).range([barChartHeight, 0]);
barChart.append("g").attr("class", "axisleft").call(d3.axisLeft(y));
...
var gridlinesy = d3
  ...
  .scale(y);
barChart.append("g").attr("class", "grid").call(gridlinesy);
...
var buttondata = [
  { nochange: 1, fiveincrease: 1.05, tenincrease: 1.1, twentyincrease: 1.2 },
];
...
function SoilData(selectUnit) {
  ...
  d3.csv("data/Soil_type.csv").then(function (data) {
    ...
    barChart
      ...
      .attr("class", "label")
      .attr("x", function (d) {
        return x(d.Soil_type) + 10;
      })
      .attr("y", function (d) {
        return y(d.Loss_rate_max - (d.Loss_rate_max - d.Loss_rate_min) / 2) - 7;
      })
      .attr("dy", "0.75em")
      .text(function (d) {
        return d.Soil_type;
  ...
  buttonArea
    ...
    .on("click", function (d) {
      ...
      selectedchange(d.twentyincrease);
      volumeData(unit);
    })
    .on("mousemove", function () {
      d3.select(this).attr("fill", "#d1d1d1");
  ...
  buttonArea.selectAll("rect").attr("fill", "white").attr("stroke", "black");
